...
open your server's url and sign in. photos load as you scroll; selected filters stay active. **Jobs** is the only place for job controls and progress: **Galerie einlesen**, **Thumbnails generieren**, **KI-Tagging**, and **Gesichtertagging**. all four start exclusively by an explicit click, pause independently and keep their checkpoints in SQLite. importing never starts analysis or preview generation. the existing on-demand preview cache remains available when viewing images.

PHP executes each processing step server-side; the open browser requests subsequent steps only for jobs it explicitly started. navigation, filtering, sorting and persons use javascript without restarting or stopping these runs. pausing lets the current step finish. after a reload or closing the browser, explicitly start again; opening a page never resumes a stored run automatically. a stored “Gestartet” state can represent another active browser or an interrupted browser; starting again replaces its run token, while a currently executing step rejects overlapping starts. a pause also stops subsequent steps in other tabs.
...
each job card has its own scrollable activity log with timestamps, processing phases, photo IDs and results. the latest 30 entries per job are retained in SQLite, survive reloads and update through the existing three-second status polling even while a step is running. logs follow new entries unless scrolled up; reopening the page shows the latest entries. resets clear only the selected job's log. cache hits are summarized per request rather than logged per photo. no source paths, credentials, AI responses or biometric data are logged. old runs have no retroactive log entries, and log polling never starts work.
...
the CLI likewise requires exactly one explicit job flag; calling it without a job does no work:

```bash
php bin/photobutler-index --scan-only
php bin/photobutler-index --tag-only --limit=50
php bin/photobutler-index --faces-only --limit=50
php bin/photobutler-index --previews-only --limit=50
```

bounded CLI runs pause at their limit and share the same progress and controls as the browser. no cron job or background service is installed.
...
job-control checks additionally use `PLAYWRIGHT_MODULE=/path/to/playwright node tests/jobs-browser.cjs`: isolated temporary photos/database, real HTTP requests and CPU inference, no external AI calls or mocked processing. provide `PHOTOBUTLER_BROWSER_ARTIFACTS` to retain desktop/mobile screenshots. successful paid-provider inference is deliberately outside this fixture test; an unconfigured provider exercises the independent error/retry path.
