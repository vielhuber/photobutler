...
final class VideoRenderer
{
    ...
    {
        $probe = json_decode($this->run([
            'ffprobe', '-v', 'error', '-protocol_whitelist', 'file,pipe', '-select_streams', 'v:0',
            '-show_entries', 'stream=width,height:stream_side_data=rotation', '-of', 'json', $source
        ...
        $frame = $this->run([
            ...
            '-protocol_whitelist', 'file,pipe', '-i', $source, '-map', '0:v:0', '-frames:v', '1',
            '-vf', "scale='min(640,iw)':'min(640,ih)':force_original_aspect_ratio=decrease",
            '-an', '-map_metadata', '-1', '-threads', '1', '-f', 'image2pipe', '-vcodec', 'mjpeg', 'pipe:1'
        ]);
        ...
        try {
            ...
            if (is_file($target . '.tmp')) unlink($target . '.tmp');
        }
    ...
    private function run(array $command): string
    {
        ...
        if ($process === false) throw new \RuntimeException('Video-Verarbeitung nicht verfügbar.');
        try {
